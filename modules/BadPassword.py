...
with open(f"{database_directory}/projects/BadPassword.txt", "a") as f:
    f.write(project + '\n')

try:
    # Remove Line from Active, LoggedIn, TwoFactorRequired, Locked, UnknownError
    filenames = [f"{database_directory}/projects/LoggedIn.txt", f"{database_directory}/projects/TwoFactorRequired.txt", f"{database_directory}/projects/Locked.txt", f"{database_directory}/projects/UnknownError.txt"]
